"""Configuration for Redis TimeSeries writer service."""
...
# Explicit subscriptions (no wildcards)
SYMBOLS: list[str] = [
    s.strip()
...
STRATEGIES: list[str] = [
    s.strip()
    for s in os.getenv("STRATEGIES", "ema_crossover").split(",")
    if s.strip()
]
...
CURSOR_KEY_PREFIX: str = os.getenv("CURSOR_KEY_PREFIX", "timeseries:cursor")
